...
$numPhotos = $gallery->album->numPhotos(
...
do {
         ...
         isset($gallery->session->offlineAlbums[$pAlbumName])))
    {
        ...
        $breadtext[$breadCount] = _('Album') . ': <a href="' .
                makeAlbumUrl($pAlbumName) . '">' . $pAlbum->fields['title'] .
                '</a>';
    } else if (!$gallery->session->offline ||
               isset($gallery->session->offlineAlbums['albums.php']))
    {
        // We're at the top!
        $breadtext[$breadCount] = _('Gallery') . ': <a href="' .
                makeGalleryUrl('albums.php') . '">' .
                $gallery->app->galleryTitle . '</a>'; 
    } else if ($gallery->session->offline) {
        // test is redundant.  offline must be true if you reach this line.
        break;
    }
    $breadCount++;
} while ($pAlbumName);
...
<html> 
<head>
  <title><?php echo $gallery->app->galleryTitle . ' :: ' .
...
if (!isset($first)) {
?>
    <link rel="first" href="<?php
        echo makeAlbumUrl($gallery->session->albumName, '',
                array('page' => 1)) ?>" />
...
<?php
           ...
           isset($gallery->session->offlineAlbums[
                    $pAlbum->fields['parentAlbumName']]))
{
?>
...
?>
    <link rel="top" href="<?php
        echo makeGalleryUrl('albums.php',
                array('set_albumListPage' => 1)) ?>" />
<?php
}
...
if (isset($gallery->album->fields['background'])) {
    echo 'BODY { background-image:url(' .
            $gallery->album->fields['background'] .'); } ';
}
if ($gallery->album->fields['textcolor']) {
    echo 'BODY, TD {color:'. $gallery->album->fields['textcolor'] .'; }';
    echo '.head {color:'. $gallery->album->fields['textcolor'] .'; }';
    echo '.headbox {background-color:' .
            $gallery->album->fields['bgcolor'] .'; }';
}
?>
  </style>
</head>
...
function showChoice($label, $target, $args) {
    ...
    echo '<option value="' .
            makeGalleryUrl($target, $args) . "\">$label</option>";
}
...
$albums_str = pluralize_n($numAlbums,
                _('sub-album'), _('sub-albums'), _('No albums'));
$imags_str  = pluralize_n($numPhotos - $numAlbums,
                _('image'), _('images') , _('no images'));
...
if ($numAlbums && $maxPages > 1) {
    ...
    $adminText .= sprintf(_('%s in this album'), $imags_str);
}
...
if ($gallery->user->canAddToAlbum($gallery->album)) {
    $adminCommands .= popup_link('['. _('add photos') .']', 
        'add_photos.php?set_albumName=' . $gallery->session->albumName);
    $extraFields = $gallery->album->getExtraFields();
    if (!empty($extraFields)) {
        $adminCommands .= popup_link('['. _('add photo') .']',
            'add_photo.php?set_albumName=' . $gallery->session->albumName);
    }
}

if ($gallery->user->isOwnerOfAlbum($gallery->album)) {
    $adminCommands .= popup_link('[' . _('rename album') .']',
        'rename_album.php?set_albumName=' . $gallery->session->albumName .
        "&index=$i&useLoad=1");
}
    ...
    $gallery->user->canCreateSubAlbum($gallery->album))
{
    $adminCommands .= '<a href="' . doCommand('new-album',
        ...
        'view_album.php') .'">['. _('new nested album') .']</a>&nbsp;';
}
    ...
    $gallery->user->canChangeTextOfAlbum($gallery->album))
{
    $adminCommands .= popup_link('[' . _('custom fields') .']',
        'extra_fields.php?set_albumName=' . $gallery->session->albumName);
...
if ($gallery->user->canWriteToAlbum($gallery->album)) {
    if ($gallery->album->numPhotos(1)) {
        $adminCommands .= popup_link('['. _('sort') . ']',
            'sort_album.php?set_albumName='. $gallery->session->albumName);
    ...
    $adminCommands .= popup_link('[' . _('properties') . ']', 
        'edit_appearance.php?set_albumName='. $gallery->session->albumName);
}
    ...
    $gallery->user->isOwnerOfAlbum($gallery->album))
{
    $adminCommands .= popup_link('[' . _('permissions') . ']',
        'album_permissions.php?set_albumName='. $gallery->session->albumName);
}
    ...
    if ($gallery->user->isLoggedIn()) {
        $adminCommands .= '<a href="' .
            doCommand('logout', array(), 'view_album.php',
                array('page' => $page)) . '">[' . _('logout') . ']</a>';
    } else {
/*
        $adminCommands .= popup_link('['. _('login') .']',
            'login.php', 0, true, 250, 500);
 */
...
if ($numPhotos) {
    ...
    while ($rowCount < $rows) {
        ...
        while ($j <= $cols && $i <= $numPhotos) {
            ...
            list($iWidth, $iHeight) =
                    $gallery->album->getThumbDimensions($i, $scaleTo);
            if ($iWidth == 0) {
            ...
            } else if ($gallery->album->isAlbumName($i)) {
                $myAlbumName = $gallery->album->isAlbumName($i);
                $myAlbum = new Album();
                $myAlbum->load($myAlbumName);
                $gallery->html_wrap['thumbTag'] =
                        $myAlbum->getHighlightAsThumbnailTag($scaleTo);
        ...
        while ($j <= $cols && $i <= $numPhotos) {
            if ($gallery->album->isAlbumName($i)) {
                ...
                list($iWidth, $iHeight) =
                        $gallery->album->getThumbDimensions($i);
            }
            ...
            if ($showAdminForm) {
                echo makeFormIntro('view_album.php',
                        array('name' => "image_form_$i")); 
            }
            echo "<table width=$iWidth border=0 cellpadding=0 cellspacing=4>";
            echo '<tr><td><span class="caption">';
            $id = $gallery->album->getPhotoId($i);
            ...
            if ($gallery->album->isAlbumName($i)) {
                ...
                echo _('Changed: ') .
                        $myAlbum->getLastModificationDate() .'.<br>';
                echo _('Contains: ') .
                        pluralize_n($myAlbum->numPhotos(
                            $gallery->user->canWriteToAlbum($myAlbum)),
                            _('item'), _('items'), _('0 items')) .'.<br>';
                if (!strcmp($gallery->album->fields['display_clicks'], 'yes') &&
                    !$gallery->session->offline && ($myAlbum->getClicks() > 0))
                {
                    echo _('Viewed: ') . pluralize_n($myAlbum->getClicks(),
                            _('time'), _('times'), _('0 times')) . '.<br>';
                }
                echo '</span>';
            } else {
                ...
                {
                    echo _('Viewed: ') .
                            pluralize_n($gallery->album->getItemClicks($i),
                                _('time'), _('times'), _('0 times')) .'.<br>';
                }
            }
            ...
            if ($gallery->album->getItemOwnerModify() && 
                ...
                !$gallery->user->canChangeTextOfAlbum($gallery->album))
            {
                showChoice(_('Edit Text'), 'edit_caption.php',
                    array('index' => $i));
            }
            if ($gallery->album->getItemOwnerModify() && 
                $gallery->album->isItemOwner($gallery->user->getUid(), $i) && 
                ...
                !$gallery->user->canWriteToAlbum($gallery->album))
            {
                showChoice(_('Edit Thumbnail'), 'edit_thumb.php',
                    array('index' => $i));
                showChoice(_('Rotate/Flip') ." $label", 'rotate_photo.php',
                    array('index' => $i));
            }
            ...
            if ($gallery->user->canChangeTextOfAlbum($gallery->album)) {
                if ($gallery->album->isAlbumName($i)) {
                    if ($gallery->user->canChangeTextOfAlbum($myAlbum)) {   
                        showChoice(_('Edit Title'), 'edit_field.php',
                            array('set_albumName' => $myAlbum->fields['name'],
                                    'field' => 'title'));
                        showChoice(_('Edit Description'), 'edit_field.php',
                            array('set_albumName' => $myAlbum->fields['name'],
                                    'field' => 'description'));
                    }
                    if ($gallery->user->isAdmin() ||
                    ...
                    {
                        showChoice(_('Rename Album'), 'rename_album.php',
                            array('set_albumName' => $myAlbum->fields['name'],
                                    'index' => $i));
                    }
                } else {
            ...
            if ($gallery->user->canWriteToAlbum($gallery->album)) {
                    ...
                    !$gallery->album->isAlbumName($i))
                {
                    showChoice(_('Edit Thumbnail'), 'edit_thumb.php',
                        array('index' => $i));
                    showChoice(_('Rotate/Flip') ." $label",
                        'rotate_photo.php', array('index' => $i));
                }
                if (!$gallery->album->isMovie($id)) {
                    showChoice(_('Highlight') . " $label",
                        'highlight_photo.php', array('index' => $i));
                }
                if ($gallery->album->isAlbumName($i)) {
                    $myAlbumName = $gallery->album->isAlbumName($i);
                    showChoice(_('Reset Counter'), 'do_command.php',
                        array('cmd' => 'reset-album-clicks',
                                ...
                                'return' => urlencode(
                                    makeGalleryUrl('view_album.php'))));
                }

                showChoice(_('Move ') . $label, 'move_photo.php',
                    array('index' => $i));

                if ($gallery->album->isHidden($i)) {
                    showChoice(_('Show') ." $label", 'do_command.php',
                        array('cmd' => 'show', 'index' => $i));
                } else {
                    showChoice(_('Hide') ." $label", 'do_command.php',
                        array('cmd' => 'hide', 'index' => $i));
                }
            }
            ...
            if ($gallery->user->isAdmin())
            {
                showChoice(_('Change Owner'), 'photo_owner.php',
                    array('id' => $id));
            }
            if ($showAdminForm) {
                echo '</select></form>';
    ...
    echo '<td colspan=$rows align=center class="headbox">';
    if (!$gallery->session->offline &&
        $gallery->user->canAddToAlbum($gallery->album))
    {
        echo '<span class="head">'. _('Hey! Add some photos.') .'</span>';
    } else {
        echo '<span class="head">'. _('This album is empty.') .'</span>';
    }
    echo '</td></tr>';
}
    ...
    !strcmp($gallery->album->fields['public_comments'], 'yes'))
{
    ...
    echo '<span class="error">*</span><span class=fineprint>' .
          _('Comments available for this item.') .'</span>';
    echo '<br><br>';
}
