...
function gr_new_album(&$gallery, &$response, $newAlbumName, $newAlbumTitle, $newAlbumDesc) {
	...
	if ($returnVal = createNewAlbum($gallery->session->albumName, $newAlbumName, $newAlbumTitle, $newAlbumDesc)) {
...
function processFile($file, $tag, $name, $setCaption = "") {
	...
	if (isAcceptableArchive($tag)) {
		...
		foreach ($files_to_process as $current_file) {
			...
			processFile($current_file['filename'], $current_file_ext, $current_file_name, $caption, $setCaption);
		}
