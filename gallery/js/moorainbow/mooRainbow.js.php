...
var MooRainbow = new Class({
	...
	initialize: function(el, options) {
		...
		this.element.addEvent('click', function(e) {
			curColor = $(this.options.destination).value;
			var gColor = new RGBColor(curColor);
        		if (gColor.ok) {
				this.manualSet(gColor.rgb);
        		}
			this.toggle(e);
		}.bind(this));
	...
	doLayout: function() {
		...
		var ok = new Element('input', {
			'styles': {'position': 'absolute'},
			'type': 'button',
			'value': '<?php echo gTranslate('common', "Select"); ?>',
		...
		var close = new Element('input', {
			'styles': {'position': 'absolute'},
			'type': 'button',
			'value': '<?php echo gTranslate('common', "Close"); ?>',
			'class': prefix + 'closeButton'
		}).inject(box);
		...
		this.closeButton = $E('#' + idPrefix + 'closeButton');
