 ...
 *
 * $Id: lib-despam_comments.php 14282 2006-08-12 00:46:01Z jenst $
 */
...
function deleteComments() {
	...
	if (!getRequestVar('delete')) {
		...
		printf("<h3>%s</h3>",
			gTranslate('core',
				"Deleted %d spam comment.",
				"Deleted %d spam comments.",
				$removedTotal,
				gTranslate('core', "No comment deleted."),
				true
			)
		);
	}
...
function findBlacklistedComments() {
	...
	if (empty($list)) {
		...
		print "</table>\n";
		echo '<input type="hidden" name="g1_mode" value="deleteComments">';

		echo gSubmit('deleteButton', gTranslate('core', "_Delete checked comments"));
		print "</form>";
	}
}
		...
		if (empty($removed)) {
			...
			if (!$success) {
				...
				printf("<h3>%s</h3>",
					gTranslate(
						'core',
						"Deleted %d entry from blacklist.",
						"Deleted %d entries from blacklist.",
						sizeof($removed),
						"Deleted no entry from blacklist.",
						true
					)
				);
				print "<ul>";
...
function viewBlacklist() {
	...
	if (empty($blacklist['entries'])) {
		...
		echo gSubmit('submitButton', gTranslate('core', "_Update Blacklist"));
		print "\n</form>";
	}
}
...
function showAddBox() {
	...
	echo sprintf(
		gTranslate('core', "%s made a tool called %s, which is outdated in the meanwhile, but the blacklist entries are still great."),
		'Jay Allen',
		'<a href="http://www.jayallen.org/projects/mt-blacklist" target="_blank">MT-Blacklist</a>'
	);

	echo "\n<br>";
	echo sprintf(
		gTranslate('core', "Click this link to see his %sgreat blacklist%s taken from MT-Blacklist 1.6.2"),
		'<a href="'. makeGalleryUrl('help/blacklist.txt') .'" target="_blank">',
		'</a>'
	);

	echo "\n<br>";
	echo gTranslate('core', "You can just cut and paste these blacklist entries into the text box, or add new entries of your own.");
	echo "<br><textarea rows=\"10\" cols=\"80\" name=\"newBlacklistEntries\"></textarea>";
	echo "<br>";
	echo "<input type=\"hidden\" name=\"g1_mode\" value=\"updateBlacklist\">";
	echo gSubmit ('submitButton', gTranslate('core', "_Update Blacklist"));
	print "</form>";
}
